...
class FlowNet:
    ...
    def load_torch_weights(self, t7_path: str):
        """
        Load pretrained FlowNetS weights from a Torch .t7 file
        and assign them to the Keras model.

        This assumes:
        - Torch Conv weights are [out_c, in_c, kH, kW]
        - Keras Conv weights are [kH, kW, in_c, out_c]
        """

        print(f"Loading Torch weights from: {t7_path}")
        t7 = torchfile.load(t7_path)

        # Torch model stores layers sequentially
        torch_modules = t7['modules']
        ...
        def torch_to_keras_conv(w):
            """Transpose Torch conv weights to Keras format"""
            return np.transpose(w, (2, 3, 1, 0))

        torch_idx = 0

        for module in torch_modules:
            if module['__typename__'] not in (
            ...
            if name not in keras_layers:
                print(f"Skipping Torch layer not in Keras model: {name}")
                continue

            keras_layer = keras_layers[name]

            torch_weight = module['weight']
            torch_bias = module.get('bias', None)

            keras_weight = torch_to_keras_conv(torch_weight)

            if torch_bias is not None:
                keras_layer.set_weights([keras_weight, torch_bias])
            else:
...
def main():
    ...
    #             )
    flownet.load_torch_weights('/Users/oscar/Etudes/Ecole/PR/FlowNet_v1_TF2-master/src/FlowNetS_from_scratch.t7')
